...
const [rows, apps, users, fees, revs, buys, jobs, fundings] = await Promise.all([
  ...
  prisma.revenueEvent.findMany({ select: { id: true } }),
  prisma.buyback.findMany({ select: { id: true } }),
...
const REF_SETS = {
  ...
  RevenueEvent: new Set(revs.map((r) => r.id)),
  Buyback: new Set(buys.map((r) => r.id)),
