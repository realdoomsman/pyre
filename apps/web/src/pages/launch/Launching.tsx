...
import { explorerAddressUrl, explorerTxUrl, ponsUrl } from "@pyre/shared";
...
export const Launching = ({ launch, onRestart }: { launch: LaunchDraftDto; onRestart: () => void }) => {
  ...
  const receipt = useTxReceipt(launch.launchTx);
  const [dismissed, setDismissed] = useState(false);

  const ticks = useMemo<IgnitionTick[]>(() => {
    const out: IgnitionTick[] = [];
    if (launch.stakeTx) out.push({ id: "stake", label: `stake ${formatEth(launch.stakeWei)}`, detail: shortAddress(launch.stakeTx, 6), tone: "earn" });
    if (launch.walletAddress) out.push({ id: "wallet", label: "app wallet derived", detail: shortAddress(launch.walletAddress, 6) });
    if (launch.status === "LAUNCHING" || launch.launchTx || launch.status === "LIVE") out.push({ id: "fund", label: "treasury funded launch fee + gas", detail: "0.0005 ETH" });
    if (launch.launchTx) out.push({ id: "tx", label: "factory.launchToken sent", detail: shortAddress(launch.launchTx, 6), tone: "build" });
    if (receipt.data)
      out.push({
        id: "block",
    ...
    if (launch.tokenAddress) out.push({ id: "token", label: "token created", detail: shortAddress(launch.tokenAddress, 6), tone: "build" });
      ...
      <div className="flex flex-col gap-5">
        <div role="alert" className="rounded-card border border-[color-mix(in_oklab,var(--color-danger)_40%,transparent)] bg-[color-mix(in_oklab,var(--color-danger)_8%,transparent)] px-4 py-4">
          ...
          <p className="small mt-2 text-ink-2">
            {launch.stakeRefundTx ? (
              <>
                Your stake was refunded: <Address address={launch.stakeRefundTx} kind="tx" chars={6} explorerUrl={explorerTxUrl(launch.stakeRefundTx)} className="text-13" />
              </>
            ) : launch.stakeTx ? (
        ...
        {launch.launchTx && (
          <div className="flex items-center gap-2 text-13 text-ink-2">
            ...
            <Address address={launch.launchTx} kind="tx" chars={6} explorerUrl={explorerTxUrl(launch.launchTx)} />
          </div>
        )}
        <div>
      ...
      <div className="flex flex-col gap-5">
        <div role="status" className="rounded-card border border-[color-mix(in_oklab,var(--color-warn)_40%,transparent)] bg-[color-mix(in_oklab,var(--color-warn)_8%,transparent)] px-4 py-4">
          <div className="mb-1 flex items-center justify-between gap-3">
            <span className="eyebrow text-warn">Waiting on PONS</span>
            <StatusLed tone="warn" label="Retrying" />
          </div>
          <p className="text-15 text-ink">PONS is not accepting launches from this wallet right now.</p>
          <p className="small mt-2 text-ink-2">
            The launch factory answered <span className="num">canLaunch = false</span> for the app wallet. Pyre retries automatically on every scheduler pass; you do not need to stay on
            this page. Your stake stays escrowed and is returned if the launch ultimately fails.
          </p>
        </div>
        {launch.walletAddress && (
          <div className="flex items-center gap-2 text-13 text-ink-2">
            <span className="eyebrow">app wallet</span>
            <Address address={launch.walletAddress} chars={6} explorerUrl={explorerAddressUrl(launch.walletAddress)} />
          </div>
        )}
        <div>
    ...
    <div className="flex flex-col gap-5">
      <div role="status" aria-live="polite" className="flex items-center justify-between rounded-card border border-line bg-mono-bg px-4 py-3">
        <div>
          ...
          <p className="text-14 text-ink">{live ? "Live on Robinhood Chain." : launch.launchTx ? "Launch transaction sent — waiting for the block." : "Funding the app wallet and sending the PONS launch."}</p>
        </div>
      ...
      {live && (
        <div className="flex flex-wrap gap-2">
          ...
          {launch.tokenAddress && (
            <Button variant="ghost" href={ponsUrl(launch.tokenAddress)} target="_blank" rel="noreferrer noopener">
              View on PONS ↗
            </Button>
          )}
        </div>
      ...
      {launch.launchTx && (
        <Ignition
          ...
          explorerUrl={explorerTxUrl(launch.launchTx)}
          ticks={ticks}
