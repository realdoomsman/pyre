...
const TAG: Record<NonNullable<HolderDto["tag"]>, { label: string; tone: ChipTone }> = {
  ...
  liquidity: { label: "liquidity", tone: "accent" },
...
const segments = (holders: ReadonlyArray<HolderDto>): Segment[] => {
  ...
  const pool = holders.filter((h) => h.tag === "curve" || h.tag === "pool" || h.tag === "locker" || h.tag === "vault" || h.tag === "liquidity").reduce((s, h) => s + h.pct, 0);
...
export const HoldersTab = ({ app, data }: Props) => {
  ...
  const columns = useMemo<Column<HolderDto>[]>(
    () => [
      { key: "rank", header: "#", width: 40, render: (_r, i) => <span className="num text-ink-3">{i + 1}</span> },
      {
        ...
        render: (r) => (
          <span className="inline-flex items-center gap-2">
            <AddressLink address={r.address} chars={4} venue={links} />
            {r.tag && (
      ...
      { key: "units", header: "Tokens", numeric: true, render: (r) => formatTokenUnits(r.units, { decimals: venue.tokenDecimals }) },
      { key: "pct", header: "% supply", numeric: true, render: (r) => formatPct(r.pct / 100, 2) },
      { key: "value", header: "Value", numeric: true, collapse: true, render: (r) => formatUsd(BigInt(Math.round((Number(r.units) / 10 ** venue.tokenDecimals) * app.priceUsd * 1e6))) },
    ],
    [app.priceUsd, venue, links],
  );
