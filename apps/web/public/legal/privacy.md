...
- **Robinhood Chain RPC providers and Blockscout** — on-chain reads, transaction broadcasts, and holder and transaction lookups. Your browser talks to the chain only through Pyre's read-only RPC proxy, so the public RPC never sees your IP address directly.
- **PONS and Uniswap** — the launch, curve, and pool contracts your trades interact with. Contracts see wallet addresses and amounts, as any chain interaction does.
