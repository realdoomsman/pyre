...
- **Gambling.** Games of chance for money or tokens, prediction markets, lotteries, and casino mechanics — including "free" versions that upgrade to paid.
...
- **Token-gated features** are fine when the perk is a product feature — extra tools, capacity, cosmetics — and not a payout.
- **Paid features** are fine through the platform checkout (one-time, subscription, or per-request). The price charged must match the price shown.
...
2. **Reviewer gate — before every deploy.** An automated reviewer reads each change before it goes live and blocks anything that adds wallet, auth, or payment code, loads external scripts, opens raw network access, stores secrets in the browser, or violates this policy. A blocked deploy shows up in the app's public build feed.
