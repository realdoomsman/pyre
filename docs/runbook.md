...
The `reconcile` worker runs six checks every 5 minutes and writes one `ReconcileRun` row per check (visible on `/ops`): `JOBS`, `SANDBOXES`, `JOBTOKENS` (repair: settle dead builds, kill orphaned sandboxes, revoke spent tokens), `LEDGER` (report: budget vs ledger, buybacks vs revenue, fee-split sums), `FEES` (repair: escrow `Claimed` logs with no `FeeEvent` are replayed; app wallets holding unswept ETH are reported), `BURNS` (report: `Buyback.burnedUnits` vs the on-chain supply reduction). `drifted > 0` on `LEDGER` or `BURNS` needs a human; on `FEES` it usually means the runner was down while claims happened and has already been repaired. `BURN_SUPPLY_UNREADABLE` means an `App.tokenAddress` is not an ERC-20 on this chain — expected only for seeded demo rows.
