...
Pyre is a launchpad on Robinhood Chain (chain id 4663) where every coin is a PONS v2 launch that funds one app. Creator fees from the coin accrue into a build budget; an AI agent spends that budget building and shipping the app; app revenue buys back and burns the coin. Three deployable services, one Postgres, one Redis.
...
| `runner` | `@pyre/runner` | BullMQ workers: intake, launch, build (E2B sandbox + agent), PR review, fee sweep, buyback + burn, price, holders, market indexer, monitor/self-heal, scheduler, growth, reconcile. Writes the DB directly; no HTTP to the API except the Anthropic proxy used from inside sandboxes. |
...
Shared libraries: `@pyre/shared` (economics constants, zod schemas, DTO types), `@pyre/db` (Prisma client + types), `@pyre/chain` (viem chain definition, HD wallets, ETH/USDG transfers and EIP-3009 relay, PONS v2 factory/curve/escrow/hook clients, Uniswap v4 quoter and Universal Router swaps, burn + attestation, Blockscout holders, on-chain trade/candle indexing), `@pyre/app-sdk` (browser SDK bundled into every generated app).
...
Thirteen queues, registered in `apps/runner/src/index.ts`. Repeatables are upserted as BullMQ job schedulers on boot; every side-effecting pass is guarded by a Redis lock so a second runner cannot double-sweep or double-charge.
...
| `growth` | scheduler, build engine, repeatable `growth:daily` | runner | X posts, mention replies, ad tests (recorded unpublished when `X_API_*` is empty) |
| `reconcile` | repeatable, every 5 min | runner | six drift checks (below) |
...
stateDiagram-v2
  ...
  LIVE --> DORMANT: budget under $10 and no pending revenue
  DORMANT --> LIVE: swept fee lifts budget to $10, or any ETH top-up
  LIVE --> KILLED: admin kill switch
  DORMANT --> KILLED: admin kill switch
```

`App.launchPhase` mirrors the PONS `LaunchedToken.phase` and is synced by every chain pass: `0` trading on the bonding curve, `1` curve closed and liquidity moving (transient), `2` graduated to the Uniswap v4 pool (`graduatedAt` set once, `GRADUATED` event), `3` graduation failed and the raise rescued to the creator (no market). Trades and buybacks route on this value.

## Money flow

Four units, never mixed: USD as micros, ETH as wei, coin balances as 18-decimal base units, USDG as 6-decimal units. JSON carries every bigint as a decimal string.
...
flowchart TD
  ...
  CREDITS[CREDITS:app ≥ $15] -->|ETH→USDG→card| Model[Model bill]
  Users -->|USDG: checkout / fn call / holder tier / ads| Rev[RevenueEvent → pendingRevenueMicros]
  Rev -->|buyback ≥ $5| RSplit{REVENUE_SPLIT_BPS}
  RSplit -->|85%| Burn[buy on curve or v4 → token.burn → attest]
  RSplit -->|10%| PYRE
  RSplit -->|5%| Ops[OPS]
...
- **Template**: generated apps start from `template/` with lint rules that ban `fetch`, `XMLHttpRequest`, `WebSocket`, `EventSource`, `localStorage`, `eval`, `<script>` injection and `dangerouslySetInnerHTML`. Auth, wallet, and payment code is never written by the agent; it comes from `@pyre/app-sdk`.
...
- **Payments**: USDG checkouts and per-call payments are EIP-3009 `transferWithAuthorization` messages signed by the custodial wallet (10 min validity) and relayed by the treasury; the receipt is verified (`verifyErc20Transfer`: token, recipient, amount) before any revenue is recorded. Stakes and top-ups from external wallets are verified by receipt (`verifyEthTransfer`) and a transaction hash can be used once. Nothing trusts a client-reported amount. Withdrawals are capped per user per day (`WITHDRAW_DAILY_CAP_USD`).
...
- **Rate limits**: Redis sliding windows per bucket (write, launch, auth, payout, trade, rpc, proxy, checkout, …) with `RateLimit`/`Retry-After` headers; auth, payout, proposal and proxy buckets fail closed if Redis is down.
...
Runs for apps with `growthEnabled` and budget ≥ $2. Sources: app spec + last 24 h of `DEPLOY`/`COMMIT`/`MILESTONE`/`PR_MERGED` events. Outputs: changelog thread, milestone card, revive post, deploy post, ≤ 5 mention replies/day (dedupe in Redis, 7 d), and a ≤ $5 internal ad test for apps past $100 revenue. Every post is a `GROWTH_POST` feed event; when `X_API_*` is unset the post is recorded as `[X not connected]` with an empty URL. Cost = LLM usage + $0.05 per published post, debited from `budgetMicros` with a `BUILD:<appId>` ledger row.
