...
Pyre (Solana / pump.fun) → **Pyre** (Robinhood Chain / PONS v2). Same loop — a coin's creator fees fund an AI agent that builds an app — rebuilt on EVM with a new identity, a new UI, a new repo, and a full X presence. This document is the single source of truth for the cutover. It is executed by parallel agents; every wave ends with an audit gate.

> **Historical note (2026-09-22).** §0–§3 and §7 record the 2026-09-21 cutover as it was planned and shipped, including in-app USDG payments (checkout, per-call, ads) and an app-coin buyback funded by app revenue. Those were removed on 2026-09-22: apps are free, there is no in-app money, and the only buy-and-burn is of $PYRE, funded by the 25% fee share. §1 items 3, 5 and 7 are revised below; every other payment or app-revenue mention in this file is history, not the current product. Current truth: `README.md`, `docs/economics.md`, `docs/architecture.md`, §8.
...
3. **Buyback + burn** (revised 2026-09-22) every 10 min when the `PYRE_TOKEN` ledger balance ≥ $5: the treasury buys **$PYRE** — pre-grad `curve.buy(wei, minOut, treasury)`, post-grad Universal Router v4 exact-in swap ETH→token — then `token.burn(amount)`. Attestation: a zero-value self-tx from the treasury whose calldata is `0x5059524501 || sha256(ids of the fee-share ledger entries consumed)` ("PYRE" + version + hash) — verifiable on Blockscout; `PyreBurn.attestTx` stores it. Burned supply is read from `totalSupply()` delta (v2 tokens actually burn). App coins are never bought back.
4. **Stake** stays refundable, denominated in ETH: `LAUNCH_STAKE_WEI = 0.05 ETH` (≈$135) — spam control only.
5. **Apps are free** (revised 2026-09-22). No in-app payments of any kind: no checkout, subscription, per-call price, ad slot or purchase. The only coin-linked feature inside an app is the holder tier (`balanceOf` ≥ N unlocks a feature). The app SDK exposes auth, kv, functions, llm, holder gate and track.
...
7. **$PYRE** (revised 2026-09-22) is itself a PONS v2 launch made from the treasury wallet (`PYRE_TOKEN`). Its only inflow is the 25% share of every coin's creator fees, swapped and burned per item 3.
...
## 8. Status (2026-09-22) — app revenue removed

Decision: apps built on Pyre are free to use. Removed end to end: USDG checkout, per-call (x402) payments, ad slot, purchases, revenue events, the 85/10/5 revenue split, per-charge and daily caps, revenue milestones, and the app-coin buyback they funded. Kept: the 60/25/15 creator-fee split, the $PYRE buy-and-burn (25% share, $5 minimum, same attestation), holder gating, stake, build loop, bounties, governance, staking, trading. Shared/db shape published first (`REVENUE_SPLIT_BPS`, `RevenueEvent`, `Buyback`, `Purchase`, `AdImpression`, `AdCampaign`, `DailyAppCharge` and the App revenue columns dropped; migration `20260922000000_remove_app_revenue`), then api, runner, app-sdk + template (with the Pyre design tokens for built apps), web (coin page loop panel is fees → agent → app → PYRE burn; `/burns` shows $PYRE burns only), and every doc, legal page and X copy source. Rendered media that still show the old loop are listed for re-rendering in the Copy report.

## 9. Status (2026-09-22) — launch from other chains and launchpads (branch `multichain`)

Decision: a coin can launch on a second venue, **Solana · pump.fun**, behind a venue adapter (`packages/chain/src/venue.ts`: `Chain = robinhood | solana`, `Launchpad = pons_v2 | pump_fun`, `adapterFor(launchpad)` → `ponsAdapter` | `pumpAdapter`). The loop is unchanged in spirit: the agent still builds the app, fees fund it, fees burn a coin. What differs per chain: stake (`LAUNCH_STAKE_BY_CHAIN`: 0.05 ETH / 1 SOL), the 25% leg (`FEE_SPLIT_BPS_BY_CHAIN`: $PYRE buyback on Robinhood Chain; on Solana a buyback + `burnChecked` of the app's own coin from the treasury Solana wallet, `CoinBurn` rows, `COINBURN:<appId>` ledger, SPL Memo attestation `pyre:burn:v1:<sha256>`), custodial wallets (`User.solWallet` at `m/44'/501'/<index>'/0'`, apps at `m/44'/501'/<1000000 + index>'/0'`, same seed), and trading (custodial only on Solana; "Trade on pump.fun" for external wallets). Launcher payouts stay in ETH on Robinhood Chain for every coin. **$PYRE stays one coin on Robinhood Chain; any $PYRE on another chain is not us.** Every `…Wei` column now means native base units of the app's chain; DTOs carry `chain`, `launchpad`, `native`. Env: `SOLANA_RPC_URL`, `SOLANA_CLUSTER`, `SOLANA_WSS_URL`, `PUMP_LAUNCH_ENABLED` — the venue does not exist until the RPC is set, so production is untouched. Not deployed to production; Main smoke-tests a Railway `staging` environment against Solana devnet first (`docs/go-live.md` → Solana venue, `docs/runbook.md` → Enabling the pump.fun venue). Research behind the pump.fun facts (program ids, `create_v2`, 30 bps / tiered creator fees, permissionless claims, CTO risk, devnet parameters) is in the multichain research brief; docs carry the operative numbers with the caveat that pump.fun sets and can change them.
